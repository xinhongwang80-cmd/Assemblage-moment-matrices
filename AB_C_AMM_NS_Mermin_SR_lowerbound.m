...
for i = 1:lambda
    ...
    gamma_SDP_lambda{i} = zeros(length(gamma_str_complex));
...
for i = 1:lambda
    for idx = 1:length(uni_mono)
        tfMatrix = (gamma_str_real == uni_mono(idx));
        v{idx,i} = sdpvar(1,1,'hermitian','real');
        gamma_SDP_lambda{i} = gamma_SDP_lambda{i} + v{idx,i}*tfMatrix;
    end
    v{idx+1,i} = sdpvar(1,1,'hermitian','real');
    gamma_SDP_lambda{i}(1,1) = v{idx+1,i};
...
for i = 1:lambda
    ...
    sum_gamma_SDP_lambda_11 = sum_gamma_SDP_lambda_11 + gamma_SDP_lambda{i}(1,1);
            ...
            for y = 1:ny
                ...
                for i = 1:lambda
                        ...
                        sum_D_gamma_sigma_lambda{a,b,x,y} = sum_D_gamma_sigma_lambda{a,b,x,y} + ...
                            V(i,a,b,x,y)*gamma_SDP_lambda{i};
...
for i = 1:lambda
    ...
    constr = [constr, gamma_SDP_lambda{i} >= 0];
...
xlabel('Mermin inequality','FontSize',15,'Interpreter','latex');
ylabel('minimal steering robustness','FontSize',15,'Interpreter','latex');


legend({'$\ell=1$'},'FontSize',15,'Interpreter','latex','Location','northwest');
title('Mermin inequality','FontSize',15,'Interpreter','latex');
